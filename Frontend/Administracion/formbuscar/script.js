document.querySelector('.formulario form').addEventListener('submit', async function (e) {
    e.preventDefault(); // Evitar envío del formulario tradicional

    // Capturar el nombre del videojuego
    ...
    try {
        ...
        if (result.success) {
            ...
            alert(`Videojuego encontrado: \nID: ${result.data.id}\nNombre: ${result.data.nombre}\nGénero: ${result.data.genero}`);
        } else {
            alert(`Error: ${result.message}`);
        }
    } catch (error) {
        console.error('Error en la solicitud:', error);
        alert('Hubo un error al buscar el videojuego.');
    }
});
